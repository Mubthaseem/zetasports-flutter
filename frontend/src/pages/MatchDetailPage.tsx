...
export const MatchDetailPage: React.FC<Props> = ({ match, onBack }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <button
        onClick={onBack}
        className="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-zeta-blue transition-colors"
      >
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-zeta-card to-[#070b13] border border-zeta-border p-6 shadow-2xl">
        {/* Subheader: League & Venue */}
        <div className="flex items-center justify-between text-xs text-slate-400 pb-4 border-b border-zeta-border/60 mb-6">
          <div className="font-semibold text-slate-200">
            {match.competitionName} {match.round ? `• ${match.round}` : ''}
          </div>
          <div className="flex items-center gap-2">
            {isLive ? (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-zeta-live/20 text-zeta-live border border-zeta-live/40 shadow-glow-live">
                <span className="w-2 h-2 rounded-full bg-zeta-live animate-pulse"></span>
        ...
        <div className="grid grid-cols-3 items-center text-center py-2">
          ...
          <div className="flex flex-col items-center space-y-3">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-slate-900 p-2.5 flex items-center justify-center border border-slate-800 shadow-lg">
            ...
            <h2 className="text-sm sm:text-lg font-bold text-white max-w-[150px] sm:max-w-[200px] truncate">
              {match.homeTeam.name}
            </h2>
          </div>
          ...
          <div className="flex flex-col items-center space-y-1">
            <div className="font-mono text-3xl sm:text-5xl font-black tracking-tight text-white flex items-center gap-3">
              <span className={isLive ? 'text-zeta-blue' : ''}>
                {match.score.home ?? 0}
              </span>
      ...
      <div className="flex items-center justify-center gap-2 border-b border-zeta-border pb-4">
        <button
          onClick={() => setActiveTab('events')}
          className={`px-5 py-2 rounded-xl text-sm font-bold transition-all border ${
            activeTab === 'events'
              ? 'bg-zeta-blue text-black border-zeta-blue shadow-glow-blue'
      ...
      {loading ? (
        ...
        <div>
          ...
          {activeTab === 'events' && (
            <div className="max-w-2xl mx-auto bg-zeta-card rounded-2xl border border-zeta-border p-6 shadow-sm">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-6 pb-2 border-b border-zeta-border">
                ...
                <div className="relative pl-6 space-y-6 before:content-[''] before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-800">
                  {eventsData.events.map(ev => (
                    <div key={ev.id} className="relative flex items-start gap-4 text-sm">
                      <div className="absolute -left-6 mt-0.5 w-4 h-4 rounded-full bg-slate-900 border-2 border-zeta-blue flex items-center justify-center text-[10px]">
                      </div>
                      <div className="font-mono font-bold text-xs text-zeta-blue min-w-[32px]">
                        {ev.minute}'
                      </div>
                      <div className="flex items-center gap-2">
                        {renderEventIcon(ev)}
          ...
          {activeTab === 'lineups' && (
            <div>
              {lineups ? (
                ...
                <div className="p-12 text-center text-xs text-slate-400 bg-zeta-card rounded-2xl border border-zeta-border max-w-lg mx-auto">
          ...
          {/* TAB 3: Statistics */}
          {activeTab === 'stats' && (
            <div className="max-w-xl mx-auto bg-zeta-card rounded-2xl border border-zeta-border p-6 shadow-sm space-y-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 pb-2 border-b border-zeta-border">
